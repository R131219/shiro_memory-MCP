...
class F extends M{
constructor(c){super();this.d=c.memoryDir||process.env.MEMORY_DIR||'./shiro_memories';this.cache=new Map();this.ttl=c.cacheTTL||6e4;if(!fs.existsSync(this.d))fs.mkdirSync(this.d,{recursive:true})}
...
rl.on('line',async line=>{
  ...
  try{
    ...
    if(method==='initialize'){
      console.log(JSON.stringify({
        ...
        result:{
          ...
          capabilities:{
            tools:JSON.parse('{"mem_read":{"description":"read","inputSchema":{"type":"object","properties":{"memoryId":{"type":"string"}},"required":["memoryId"]}},"mem_write":{"description":"write","inputSchema":{"type":"object","properties":{"memoryId":{"type":"string"},"content":{"type":"object"}},"required":["memoryId","content"]}},"mem_search":{"description":"search","inputSchema":{"type":"object","properties":{"keyword":{"type":"string"}},"required":["keyword"]}},"mem_delete":{"description":"delete","inputSchema":{"type":"object","properties":{"memoryId":{"type":"string"}},"required":["memoryId"]}},"mem_list":{"description":"list","inputSchema":{"type":"object","properties":{}}},"mem_stats":{"description":"stats","inputSchema":{"type":"object","properties":{}}},"mem_context":{"description":"context","inputSchema":{"type":"object","properties":{}}}}')
          },
    ...
    switch(method){
      case'tools/call':
        switch(params.name){
          case'mem_read':r=await srv.pr(params.arguments?.memoryId);break;
          case'mem_write':r=await srv.pw(params.arguments?.memoryId,params.arguments?.content);break;
          case'mem_search':r=await srv.ps(params.arguments?.keyword);break;
          case'mem_delete':r=await srv.pd(params.arguments?.memoryId);break;
          case'mem_list':r=await srv.pl();break;
          case'mem_stats':r=await srv.pst();break;
          case'mem_context':r=ok({server:'shiro_memory',version:'2.0.0',status:'running',memoriesDir:this.a.d});break;
          default:r=fail('unknown tool:'+params.name);
        }
        r = r || {success:false,error:'no result',code:'EMPTY'};
        break;
